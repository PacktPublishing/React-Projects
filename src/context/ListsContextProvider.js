...
const ListsContextProvider = ({ children }) => {
  ...
  const getListsRequest = async () => {
    const result = await fetchData('https://my-json-server.typicode.com/pranayfpackt/-React-Projects/lists');
  ...
  const getListRequest = async (id) => {
    const result = await fetchData(`https://my-json-server.typicode.com/pranayfpackt/-React-Projects/lists/${id}`);
